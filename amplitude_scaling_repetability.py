...
A_a_mod, bin_edges_A_a_mod, *_ = stats.binned_statistic(t_amp_a, A_a, 'mean', bins=nnn)
A_a_mod_err, *_ = stats.binned_statistic(t_amp_a, A_a, 'std', bins=nnn)
t_amp_a_mod = np.diff(bin_edges_A_a_mod)/2 + bin_edges_A_a_mod[0:-1]
...
A_b_mod, bin_edges_A_b_mod, *_ = stats.binned_statistic(t_amp_b, A_b, 'mean', bins=nnn)
A_b_mod_err, *_ = stats.binned_statistic(t_amp_b, A_b, 'std', bins=nnn)
t_amp_b_mod = np.diff(bin_edges_A_b_mod)/2 + bin_edges_A_b_mod[0:-1]
...
A_c_mod, bin_edges_A_c_mod, *_ = stats.binned_statistic(t_amp_c, A_c, 'mean', bins=nnn)
A_c_mod_err, *_ = stats.binned_statistic(t_amp_c, A_c, 'std', bins=nnn)
t_amp_c_mod = np.diff(bin_edges_A_c_mod)/2 + bin_edges_A_c_mod[0:-1]
...
plt.errorbar((t_amp_a_mod[1:-1]-t_amp_a_mod[0])/((h0*mu)/sigma), (A_a_mod[1:-1]*(10**-9))/((A_a_mod[0])*(10**-9)), (A_a_mod_err[1:-1]*(10**-9))/((A_a_mod[0])*(10**-9)), fmt='o', solid_capstyle='projecting', capsize=5, marker=mark, color=col, label=r"%02i $\mu m$, %03i mPa.s" %(round(h0*(10**6)), mu*(10**3)))
# plt.errorbar((t_amp_b_mod[1:-1]-t_amp_b_mod[0])/((h0*mu)/sigma), (A_b_mod[1:-1]*(10**-9))/((A_b_mod[0])*(10**-9)), (A_b_mod_err[1:-1]*(10**-9))/((A_b_mod[0])*(10**-9)), fmt='o', solid_capstyle='projecting', capsize=5, marker=mark, color=col, label=r"%02i $\mu m$, %03i mPa.s" %(round(h0*(10**6)), mu*(10**3)))
plt.errorbar((t_amp_c_mod[1:-1]-t_amp_c_mod[0])/((h0*mu)/sigma), (A_c_mod[1:-1]*(10**-9))/((A_c_mod[0])*(10**-9)), (A_c_mod_err[1:-1]*(10**-9))/((A_c_mod[0])*(10**-9)), fmt='o', solid_capstyle='projecting', capsize=5, marker=mark, color=col, label=r"%02i $\mu m$, %03i mPa.s" %(round(h0*(10**6)), mu*(10**3)))
