...
f = r'/media/devici/Samsung_T5/srinath_dhm/impact_over_thin_films/speed1/00200cs0005mum_r3/2018.09.09 20-27'
...
def_files = sorted(glob.glob('*_unwrapped.txt'), key=os.path.getmtime)
...
theta = 22.5                                                                    #central angle
theta_inc = 45 + 22.5                                                           #max increment in central angle
# theta_inc = 1                                                                 #max increment in central angle
delta_theta = 0.5                                                               #increment in angle
p = 1                                                                           #skip files
smth = 20                                                                       #smooth parameter
nn = 100                                                                        #ignore profile for index value below
...
rr = np.arange(0,s)*conv
np.savetxt('r.txt', rr, delimiter='\t')

# print(t[0])
# print(t[1999])
# print(np.arange(img2+1,n,p))
# print(n)
# input()
...
for i in np.arange(img2+1,n,p):
    ...
    prof = smooth_theta_profiles*(10**(-9))
    np.savetxt('h_%05i.txt' %i, prof, delimiter='\t')
    ...
    # print(h_min[count])
    # print(h_max[count])
    # print(abs(h_max[count]) + abs(h_min[count]))
...
ax.plot(time, L_fit, c='red', label=r'$L_{\lambda} = $ %i $t^{%0.3f}$' %(initial_wav, power_wav))
handles,labels = ax.get_legend_handles_labels()
handles = [handles[1], handles[0]]
labels = [labels[1], labels[0]]
ax.legend(handles,labels)
