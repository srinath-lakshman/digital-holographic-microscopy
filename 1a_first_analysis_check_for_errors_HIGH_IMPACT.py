...
def deformation_center_new(image, x_c, y_c, s1, start_angle, end_angle):
    ...
    n = int(round((end_angle - start_angle)/delta_theta))                       #n = 360
    ...
    for k in range(start_angle,end_angle):
        ...
        for j in range(0,s1):
            ...
            print(xx, yy)
            ph[count,j] = image[int(round(yy)),int(round(xx))]
            # ph[count,j] = 1
        input()
...
f = r'/media/devici/srinath_dhm02/srinath_dhm/impact_over_thin_films/speed2/00200cs0010mum_r3/2018.09.10 20-21'
...
# x_max, y_max, xc, yc, delta_theta = deformation_center_new(image, 750, 450, 300, 90, 270)
#
