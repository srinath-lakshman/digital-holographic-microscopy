...
if __name__ == "__main__":
    ...
    # print(cl_args)
    # print(cl_args.nref)
    # print(cl_args.nsample)
    # print(cl_args.path)
    # print(cl_args.transmission)

    phase_path = os.path.join(cl_args.path, 'Phase/Image/')

    #phase_images = os.listdir(phase_path)
    phase_images = os.listdir(r'C:\Users\LakshmanS\Desktop\hahaha\2018.09.09 16-53\Phase\Image')
    #print(phase_path)
    ...
    ll = np.arange(0,4*np.pi,0.1*np.pi)

    mm = np.angle(np.exp(1j * ll))
    # plt.xticks([0, 4*np.pi])
    ...
    for file_name in phase_images:
        if not file_name.startswith('.') and os.path.isfile(os.path.join(phase_path, file_name)):
            ...
            print(os.path.join(phase_path, file_name))
            input()
            ...
            # yoyo1 = os.path.join(phase_path, file_name)
            # yoyo2 = Image.open(yoyo1)
            # im = np.asarray(yoyo2)
            #print(type(im))
            #plt.imshow(yoyo2,'gray')
