...
for i in np.arange(img2+1,n,p):
    ...
    # print(len(smooth_theta_profiles))
    # input()

    index_min[count] = np.argmin(smooth_theta_profiles[start_point:end_point]) + start_point
    index_max[count] = np.argmax(smooth_theta_profiles[start_point:end_point]) + start_point

    h_min[count] = min(smooth_theta_profiles[start_point:end_point])
    h_max[count] = max(smooth_theta_profiles[start_point:end_point])
    ...
    # plt.plot(theta_profiles, c='#ff7f0e')                                       #orange
    # plt.plot(smooth_theta_profiles, c='#1f77b4')                                #blue
...
L = abs(abs(index_max))*conv*np.power(10,6)
